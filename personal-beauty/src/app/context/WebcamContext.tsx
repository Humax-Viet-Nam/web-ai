...
interface WebcamContextType {
  ...
  setCountdownActive: React.Dispatch<React.SetStateAction<boolean>>;
  capturedLandmarks: NormalizedLandmark[];
  startCountdown: (canvasRef: RefObject<HTMLCanvasElement | null>) => void;
  resetCountdown: () => void;
...
export const WebcamProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const detectFull = useCallback(
    (landmarks: any[], isIndexRaised: boolean) => {
      ...
      if (
        (isFist || predictedFist) &&
        (preFistFreezePosition.current || lastPositionBeforeFist.current)
      ) {
        outputPosition =
          preFistFreezePosition.current ||
  ...
  const captureImage = useCallback(
    async (canvasRef: RefObject<HTMLCanvasElement | null>) => {
